...
class BiquadFilter:
    ...
    def process(self, input_wave):
        ...
        modulated_cutoff = self.base_cutoff_hz
        ...
        if self.envelope is not None:
            ...
            octave_range = 5.0
            modulated_cutoff = modulated_cutoff * (2 ** (env_signal * octave_range * self.envelope_depth))

        if self.lfo_instance is not None:
            lfo_signal = self.lfo_instance.process(num_samples) 
            octave_range = self.cutoff_mod_depth * 1.0
