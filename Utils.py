...
PARAM_NAMES = [
    ...
    'lfo1_rate', 'lfo1_shape', 'lfo1_pdepth', 'lfo1_vdepth',
    'lfo2_rate', 'lfo2_shape', 'cutoff_mod_depth',
...
def load_parameters_file(name):
    ...
    for preset in parameters_file:
        ...
        lfo1_pdepth.append(preset['lfo1']['pitch_mod_depth'])
        lfo1_vdepth.append(preset['lfo1']['volume_mod_depth'])

        lfo2_rate.append(preset['lfo2']['rate'])
        lfo2_shape.append(preset['lfo2']['shape'])
        cutoff_mod_depth.append(preset['lfo2']['cutoff_mod_depth'])
    ...
    return {
        ...
        'lfo1_pdepth': np.array(lfo1_pdepth),
        'lfo1_vdepth': np.array(lfo1_vdepth),
...
def mel_spectrogram(audios, sr, n_fft=2048, hop_length=512, n_mels=128):
    ...
    for y in audios:
        ...
        # S_smooth = scipy.ndimage.gaussian_filter(S_norm, sigma=(1, 1))
...
def manage_normalization(presets, should_normalize):
    ...
    return {
        # LFO 1 y 2
        "lfo1_rate": fun(presets['lfo1_rate'], 0.000001, 10),
        "lfo1_shape": fun(presets['lfo1_shape'], 0, 4),
        "lfo1_pdepth": fun(presets['lfo1_pdepth'], 0, 1),
        "lfo1_vdepth": fun(presets['lfo1_vdepth'], 0, 1),

        "lfo2_rate": fun(presets['lfo2_rate'], 0.000001, 10),
